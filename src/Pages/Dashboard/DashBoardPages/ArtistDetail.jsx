...
const ArtistDetail = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/artist/products/artist/${id}`);
        setProducts(response.data); // Set fetched products to state
      } catch (err) {
        console.error("Error fetching artist products:", err);
        toastr.error("Failed to fetch products for the artist");
      } finally {
        setLoading(false);
      }
    };
  ...
  const deleteProduct = async (productId) => {
    try {
      ...
      if (confirmDelete) {
        await axios.delete(`http://localhost:3001/artist/products/${productId}`);
  ...
  const updateProduct = async (productId) => {
    ...
    const updatedData = {
      productName: prompt("Enter new product name", productToUpdate.productName),
      size: prompt("Enter new size", productToUpdate.size),
      description: prompt("Enter new description", productToUpdate.description),
      newPrice: prompt("Enter new price", productToUpdate.newPrice),
    };

    try {
      const response = await axios.put(`http://localhost:3001/artist/products/${productId}`, updatedData);
      const updatedProduct = response.data;
      setProducts(products.map(product => product._id === productId ? updatedProduct : product)); // Update product in state
                            ...
                            {loading ? (
                              ...
                              products.map((product) => (
                                <tr key={product._id}>
                                  ...
                                  <td>
                                    <button type="button" className="btn btn-info" title="Edit" onClick={() => updateProduct(product._id)}>
                                      <i className="fa fa-edit"></i>
                                    </button>
                                    <button type="button" className="btn btn-danger js-sweetalert" title="Delete" onClick={() => deleteProduct(product._id)}>

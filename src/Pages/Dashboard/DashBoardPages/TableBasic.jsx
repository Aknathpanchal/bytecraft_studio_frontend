...
function TableBasic() {
    ...
    const handleUpdateArtist = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:3001/artist/artists/${editingArtist._id}`, artistForm);
            console.log('Update response:', response);
            setArtists(artists.map(artist => (artist._id === editingArtist._id ? { ...artist, ...artistForm } : artist)));
    ...
    if (error) {
        return (
            <div className="alert alert-danger" role="alert">
                {error} <button onClick={fetchArtists} className="btn btn-primary">Retry</button>
            </div>
        );
    }
                                                ...
                                                {artists.map((artist) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td>
                                                            ...
                                                            <p className="c_name">{artist.name} {artist.lastName && <span className="badge badge-default m-l-10 hidden-sm-down">Family</span>}</p>
                                                        </td>
                                                        <td>
                                                            <span className="phone"><i className="zmdi zmdi-phone m-r-10"></i>{artist.phone || artist.email}</span>
                                                        </td>
                                                        <td>
                                                            <address><i className="zmdi zmdi-pin"></i>{artist.address.line1}, {artist.address.city}</address>
                                                        </td>
                                                        <td>
                                                            <button
                                                                type="button"
                        ...
                        <div className="modal-content">
                            ...
                            <form onSubmit={handleUpdateArtist}>
                                <div className="modal-body">
                                    <input type="text" name="name" value={artistForm.name} onChange={handleFormChange} placeholder="First Name" />
                                    <input type="text" name="lastName" value={artistForm.lastName} onChange={handleFormChange} placeholder="Last Name" />
                                    <input type="text" name="phone" value={artistForm.phone} onChange={handleFormChange} placeholder="Phone" />
                                    <input type="email" name="email" value={artistForm.email} onChange={handleFormChange} placeholder="Email" />
                                    <input type="text" name="address.line1" value={artistForm.address.line1} onChange={handleFormChange} placeholder="Address Line 1" />
                                    <input type="text" name="address.city" value={artistForm.address.city} onChange={handleFormChange} placeholder="City" />
                                </div>
